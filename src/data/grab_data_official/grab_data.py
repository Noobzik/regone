...
class Train:
    ...
    """
    def __init__(self, num: str, miss: str, date: str, mode: str, term: str, time_recorded: pendulum.DateTime, etat: str = ""):
        self.num = num
        self.miss = miss
        self.date = date
        self.mode = mode
    ...
    def get_train(self):
        ...
        """
        return self.time_recorded, self.num, self.miss, self.date, self.mode, self.term, self.direction, self.etat

    def to_dict(self):
        """
        Convertie la class train en un dictionnaire
        """
        a = ["time_recorded", str(self.time_recorded),
            "num", self.num,
             "miss", self.miss,
            "date", self.date,
            "mode", self.mode,
        ...
        if size != 0:
            for i in xml_df['train']:
                if re.match(_pattern, i["num"]):
                    if "etat" in i:
                        t = RERB(i["num"], i["miss"], i["date"]["$"], i["date"]["@mode"], i["term"], date, i["etat"])
                    else:
...
def save_df(data: list[list[dict]], gares: list[str], datetime_obj: pendulum.DateTime):
    """
    Méthode qui va sauvegarder les données sur un CSV en local
    """
    for i in range(len(data)):
        df = pd.DataFrame.from_dict(data[i])
        df = df.assign(time_recorded=datetime_obj)
...
def save_to_s3(gares: list[str], datetime_obj: pendulum.DateTime):
    """
...
if __name__ == '__main__':
    """
    Ce fichier est utilisé pour une production en local, sans utiliser Airflow
    """
    main()
